...
async def create(ctx, name=None):
	...
	if user_id not in db["characters"] or not db["characters"][user_id]:
		...
		await ctx.message.reply(f"You have already created your character.")
...
async def skill(ctx):
	...
	if character.level > 5:
		pass # add skill creation logic here
...
@bot.command(name="clear", help="[DEV] Clear database for suicide")
async def clear(ctx):
	db.clear()
	await ctx.message.reply(f"Database cleared, go burn")

@bot.command(name="database", help="[DEV] prints data base")
async def database(ctx):
	await ctx.message.reply(db["characters"])
